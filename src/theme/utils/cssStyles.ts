...
export default function cssStyles(theme: ThemeOptions) {
  return {
    bgBlur: (props?: any) => {
      const color = props?.color || (theme as any)?.palette.background.default || '#000000'
    ...
    bgImage: (props: any) => {
      ...
      const direction = getDirection(props?.direction)
      const startColor = props?.startColor || alpha((theme as any)?.palette.grey[900] || '#000000', 0.88)
      const endColor = props?.endColor || alpha((theme as any)?.palette.grey[900] || '#000000', 0.88)
